...
void ResultRenderer::init(const std::map<std::string, std::any>& properties, const std::map<std::string, InPad*>& in_pads, const std::map<std::string, OutPad*>& out_pads)
{
...
void ResultRenderer::process(const std::map<std::string, InPad*>& in_pads, const std::map<std::string, OutPad*>& out_pads)
{
    ...
    auto det_results = std::any_cast<std::vector<DetectorResult>>(buffer->get_buffer("det_results"));
